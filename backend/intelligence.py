...
def charger_keywords_db():
    try:
        ...
        print("❌ Error loading keywords DB:", e)
        return []
...
def analyse_contenu_sensible(file_path):
    try:
        ...
        print("🧠 ANALYZED TEXT (start):")
        print(text[:300])
        print("📚 Loaded keywords:", mots_cibles)

        detections = [mot for mot in mots_cibles if mot in lower_text]
        print("📌 Detected keywords in text:", detections)

        est_sensible = len(detections) > 0
        message = "Local analysis only\n"
        if est_sensible:
            message += f"✅ Sensitive data detected\n🟡 Words: {', '.join(detections)}"
        else:
            message += "❌ No sensitive words detected locally."
